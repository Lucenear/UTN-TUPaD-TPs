...
# --- Consigna 8: Muestreo aleatorio simple y comparacion de medias ---
cat("\n---> Consigna 8: Muestreo aleatorio simple y comparacion de medias\n")

# Media poblacional del peso
mu_peso <- mean(datos$`PESO KG.`, na.rm = TRUE)
round(mu_peso, 2)
...
cat("\nMedias: --> 6 muestras aleatorias(n=20)\n")
for(i in 1:n_muestras) {
  muestra <- sample(datos$`PESO KG.`, tam_muestra, replace = FALSE)
  media_muestra <- mean(muestra)
  medias_muestrales[i] <- media_muestra
  cat("Muestra", i, ": Promedio de peso =", round(media_muestra, 2), "kg\n")
}

# Muestro resumen
cat("\nResumen de medias muestrales:\n")
print(round(medias_muestrales, 2))
cat("Media promedio de las 6 muestras:", round(mean(medias_muestrales), 2), "kg\n")
cat("Desvio estandar de las medias muestrales:", round(sd(medias_muestrales), 2), "kg\n")

# Grafico de medias muestrales vs media poblacional
plot(1:6, medias_muestrales, type = "b", col = "steelblue",
     ylim = c(61, 67),
     xlab = "Muestra", ylab = "Peso promedio (kg)",
     main = "Medias de 6 muestras vs Media poblacional")
abline(h = mu_peso, col = "red", lwd = 2, lty = 2)
legend("topright", legend = c("Media muestral", "Media poblacional"),
       col = c("steelblue", "red"), lty = c(1, 2), pch = c(1, NA))